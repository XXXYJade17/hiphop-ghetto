...
@AllArgsConstructor
@Slf4j
public class SongCollectionCountIncreaseHandler implements IStrategy<StatsOperationEvent>{

    private final SongRepository songRepository;

    @Override
    public void handle(StatsOperationEvent event) {
        songRepository.increaseCollectionCount(event.getResourceId().toString());
    }
