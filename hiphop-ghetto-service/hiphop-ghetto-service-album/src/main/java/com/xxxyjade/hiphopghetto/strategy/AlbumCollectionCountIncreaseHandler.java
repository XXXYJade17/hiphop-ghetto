...
@AllArgsConstructor
@Slf4j
public class AlbumCollectionCountIncreaseHandler implements IStrategy<StatsOperationEvent>{

    private final IAlbumService albumService;

    @Override
    public void handle(StatsOperationEvent event) {
        albumService.increaseCollectionCount(event.getResourceId().toString());
    }
