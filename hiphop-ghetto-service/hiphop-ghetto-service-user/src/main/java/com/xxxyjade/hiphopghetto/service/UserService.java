...
//        }
        ...
        return UserAuthVO.builder()
                .id(user.getId())
                .token(JwtUtil.createAuthJwt(user))    // 生成令牌
                .build();
    }
    ...
    public UserAuthVO login(UserLoginDTO userLoginDTO) {
        ...
        return UserAuthVO.builder()
                .id(user.getId())
                .token(JwtUtil.createAuthJwt(user))
                .build();
    }
    ...
    @ThreeLevelCacheEvict(key = "'user::id=' + #statsUpdateDTO.id")
    public void updateStats(StatsUpdateDTO statsUpdateDTO) {
        userMapper.updateStats(statsUpdateDTO.getId(), statsUpdateDTO.getStatsType(), statsUpdateDTO.getValue());
    }
